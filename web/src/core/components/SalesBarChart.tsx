...
export function SalesBarChart({ series, granularity, title, isLoading }: SalesBarChartProps) {
    ...
    const option: echarts.EChartsOption = {
      tooltip: {
        ...
        formatter: (params: echarts.DefaultLabelFormatterCallbackParams | echarts.DefaultLabelFormatterCallbackParams[]) => {
          type TooltipItem = { axisValueLabel?: string; marker?: string; seriesName?: string; value?: number };
          const items = (Array.isArray(params) ? params : [params]) as TooltipItem[];
          let html = `<strong>${items[0]?.axisValueLabel ?? ''}</strong><br/>`;
          for (const item of items) {
            html += `${item.marker ?? ''} ${item.seriesName}: <strong>$ ${formatCurrency(Number(item.value))}</strong><br/>`;
          }
          return html;
        },
      },
  ...
  return (
    <div>
      <h3 className="text-xs font-semibold text-center uppercase tracking-widest text-zinc-700 mb-2">
        {title}
      </h3>
